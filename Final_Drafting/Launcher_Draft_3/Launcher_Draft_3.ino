...
// Also implements refactoring and readability improvements from previous launcher versions

// Base Complete System Code: This code does full functionality for the launcher. Any modifications made from here are improvements
// to this base.
...
bool hit = false;                   // Set UI to HIT until user lets go of button                                                                                         (returns to idel state upon letting up on trigger)
...
{  
  ...
  else if(targeting)                                    // Firing State
  {
    ...
    // Check button
    if(digitalRead(triggerButton)==HIGH)               // If button held, TX targeting packet
    {      
      ...
      { 
        if (rf95.recv(buf, &len))           // RX successfully within timeout (RX read into buf)
        {
          ...
          // Decide Hit or Miss
          if(incomingPacket[0] == 'H')      // Hit        (Note: Needs '' to denote CHAR here, do not use "")
          {
...
// Set LED colors using global variables (Be careful with side effects!)
