        ...
        { 
          if (rf95.recv(buf, &len))           // RX successfully within timeout (RX read into buf)
          {
            ...
            if(incomingPacket[0] == 'H')      // Hit - check first char of received message, will either be 'H' or 'M'        (Note: Needs '' to denote CHAR here, do not use "")
            {
              ...
              tone(SPEAKER, (NUM_LEDS+1)*speakerFreqConstant, 200);       // All LEDs active so at max freq here  
            }
...
void resetLEDs()
{
  ...
  //timingPotAdjustedValue = (timingPotRawValue / 140) + 3;       // Translates analog input to seconds (3-10)
  
  timingPotAdjustedValue = (timingPotRawValue / 102);           // Translates analog input to seconds (0-10)
...
{
  colorChangeLocation = NUM_LEDS - colorChangeLocation - 2;
  
      // RED (inactive)
  for(int i = 0; i <= colorChangeLocation; i++)
  {
    leds[i] = CRGB(r2, g2, b2);                // Set given LED to RGB 1 
  }
    // BLUE (active, pre-counted)
  for(int i = colorChangeLocation+1; i <= (NUM_LEDS-1); i++)
  {
    leds[i] = CRGB(r1, g1, b1);                // Set given LED to RGB 2
  }
