    ...
    {      
      ...
      { 
        if (rf95.recv(buf, &len))           // RX successfully within timeout (RX read into buf)
        {
          ...
          // TODO: Parse buffer, decide if hit or miss
          // If hit, increment hit counter until we go to hit state
          // Need to decide on hit state behivior, may want to just implement hit logic here, or we can give it its own case statement depending on desired behavior
        }
