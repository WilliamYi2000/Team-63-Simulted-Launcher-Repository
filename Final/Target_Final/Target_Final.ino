...
#define TimeoutPeriod 2000    // Time (ms) for a message to timeout
#define TX_Delay      400     // Time (ms) we delay before sending an ACK after senind a message
...
int cur_cal_max = 9;                      // Calibration iterator upper bound; should be one less than cal_max_num (since we start at 0, 10 = 0-9)
...
int universalDelay = 100;       // Delay of every loop in ms; for idle loops this will be our only delay
int timeOutLoops = 10;          // Timeout time will be universalDelay * timeOutLoops
int idleTime = 0;               // Stores the amount of loops we've been idle; +1 for every loop we don't recieve a packet, reset to 0 whenever we recieve a packet - target times out when this equals the above "TimeoutLoops"
...
#define NUM_LEDS    40                              // Total number of LEDs in LED strip
...
int ledsPerPacket = (NUM_LEDS / cal_max_num);       // Number of LEDs that light up per packet received
...
{  
  ...
  if (rf95_1.available() && rf95_2.available() && rf95_3.available() && rf95_4.available())
  {
    ...
    if (rf95_1.recv(buf_1, &len_1) && rf95_2.recv(buf_2, &len_2)  && rf95_3.recv(buf_3, &len_3) && rf95_4.recv(buf_4, &len_4))
    {
      ...
      {
        // Set cal number based on what we get from launcher
        char passedCalNum = incomingPacket_1[1];              // Get hit num from message; should be second character         // Warning: Only reads one char!
        int calNum = passedCalNum - '0';                      // Convert from char to int
        cur_cal_num = calNum;                                 // THIS WILL NOT WORK with values greater than 10
        ...
        if(cur_cal_num >= cur_cal_max)          // We've sucessfully calibrated! Ready to target
        {
          // LEDs should finish blue and yellow
          lightLEDsTwoColors(((NUM_LEDS_LAUNCHER-cur_cal_num)*ledsPerPacket), 255, 255, 0, 0, 0, 255);        // Yellow, Blue 
          ...
          // All LEDs flash blue/green once packet sent
      ...
      else if(incomingPacket_1[0] == 'T')         // Targeting packet
      {
        ...
        if(!firstCalibrationDone)                           // We have NOT done a calibration phase, we can't target! Need to do that first!       
        {
          ...
          // TX response packet to launcher
          // Response packet = B                          // BAD
          char radiopacket[9] = "B NoCal ";               // Build  packet
          rf95_1.send((uint8_t *)radiopacket, 8);         // SEND PACKET  (8 characters)
          delay(10);                                      // Short delay for safety
          rf95_1.waitPacketSent();                        // Pause until packet fully sent
        }
        else                                                // We have done at least one calibration phase - can continue with targeting as normal
        {
          ...
          String r_status_pkt = "";             // Stores which radios hit and which miss, to send to launcher if we read a miss
          ...
          {  
            ...
            char passedHitNum = incomingPacket_1[1];              // Get hit num from message; should be second character         // Warning: Only reads one char!
            ...
            // Response packet = H + radio status                             // Hit
            r_status_pkt = "H | " + r_status_pkt;                             // Add 'H'
            const char *r_status_pkt_send = r_status_pkt.c_str();             // Sends H + which radios hit and which miss
            rf95_1.send((uint8_t *)r_status_pkt_send, 16);                    // SEND PACKET  (15 characters)
            delay(10);                                                        // Short delay for safety
            rf95_1.waitPacketSent();                                          // Pause until packet fully sent
          ...
          else                                                            // MISS
          {
            ...
            // TX response packet to launcher
            // Response packet = M + radio status                             // Miss
            r_status_pkt = "M | " + r_status_pkt;                             // Add 'M'
            const char *r_status_pkt_send = r_status_pkt.c_str();             // Sends M + which radios hit and which miss
            rf95_1.send((uint8_t *)r_status_pkt_send, 16);                    // SEND PACKET  (15 characters)
            delay(10);                                                        // Short delay for safety
            rf95_1.waitPacketSent();                                          // Pause until packet fully sent
  ...
  else if(idleTime >= timeOutLoops)               // Timeout        // TODO: Check # of loops to trigger this (timing)
  {
    ...
    if(firstCalibrationDone)            // If we've done at least one calibration, print calibration data
    {
        // Radio 1 and 2
      display.setCursor(10,10);
      display.print(F("R1C: "));
      display.print(r1_targeting_min);
      display.print(F("  R2C: "));
      display.print(r2_targeting_min);
        // Radio 3 and 4
      display.setCursor(10,20);
      display.print(F("R3C: "));
      display.print(r3_targeting_min);
      display.print(F("  R4C: "));
      display.print(r4_targeting_min);
    }
  ...
  // Universal delay
  delay(universalDelay);                     // Adjust this if necessary
...
void lightLEDsTwoColors(int colorChangeLocation, int r1, int g1, int b1, int r2, int g2, int b2)
{
  ...
  for(int i = colorChangeLocation; i <= (NUM_LEDS-1); i++)
  {
