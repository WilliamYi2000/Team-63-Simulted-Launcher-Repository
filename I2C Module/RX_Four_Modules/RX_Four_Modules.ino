...
void setup() 
{
  ...
  pinMode(RFM95_4_RST, OUTPUT);
  digitalWrite(RFM95_4_RST, HIGH);
  ...
  while (!rf95_4.init()) 
  {
    Serial.println("LoRa radio 4 init failed");

    display.clearDisplay();
    display.setCursor(10, 20);
    display.print(F("Radio 4 Init FAIL"));  
    display.display();
    
    while (1);
  }
  display.setCursor(10, 30);                 // Set screen cursor to position 4
  display.print(F("Radio 4 Init good"));  
  display.display();
  ...
  rf95_4.setTxPower(23, false);   // Set to 23 dBm (default is 13 dBm)
...
//      RH_RF95::printBuffer("Received from 2: ", buf_2, len_2);
      ...
      Serial.print("Got 4: ");
      Serial.println((char*)buf_4);

      // Print to screen
      display.setCursor(10, 0);                 // Set screen cursor to position 1
      display.print(F("1: "));               
      display.print(rf95_1.lastRssi(), DEC);  

      display.print(F("  2: "));
      display.println(rf95_2.lastRssi(), DEC);  

      display.setCursor(10, 10);                // Set screen cursor to position 3
      display.print(F("3: "));
      display.print(rf95_3.lastRssi(), DEC);

      display.print(F("  4: "));
      display.println(rf95_4.lastRssi(), DEC);
    }
    else
    {
