// Target Draft 7
// Mk 7 of complete launcher system
...
int timeOutLoops = 12;          // Timeout time will be universalDelay * timeOutLoops
    ...
    if (rf95_1.recv(buf_1, &len_1) && rf95_2.recv(buf_2, &len_2)  && rf95_3.recv(buf_3, &len_3) && rf95_4.recv(buf_4, &len_4))
    {
      ...
      else if(incomingPacket_1[0] == 'T')         // Targeting packet
      {
        ...
        else                                                // We have done at least one calibration phase - can continue with targeting as normal
        {
          ...
          String miss_pkt = "M | ";             // Stores which radios hit and which miss, to send to launcher if we read a miss
          ...
          {  
           ...
           display.print(hitNum);             
            ...
            char radiopacket[2] = "H";                      // Build response packet
          ...
          else                                                            // MISS
          {
            ...
            const char *miss_pkt_send = miss_pkt.c_str();   // Sends M + which radios hit and which miss
            rf95_1.send((uint8_t *)miss_pkt_send, 16);      // SEND PACKET  (15 characters)
            delay(10);                                      // Short delay for safety
            rf95_1.waitPacketSent();                        // Pause until packet fully sent
          }
        }
